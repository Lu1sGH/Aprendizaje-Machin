...
def bgd(data, alfa=0.01, maxIt=10, variables=None, mono=True):
    ...
    if mono:
        ...
        n = len(y)
        while a < maxIt:
            #w[1] = w[1] - ( - alfa * (2/n) * np.sum(x*(y-(w[0]+w[1]*x))))
...
w1, todos_W1 = bgd(test1, maxIt=5, alfa=0.000001, variables=train1.columns, mono=True)
print("\nTest: \n", test1)
print("\nPredicción: \n")
acc = 0
for i in todos_W1:
...
#MULTIVARIABLE
print("\n","#"*25, "Multivariable", "#"*25)
w2, todos_W2 = bgd(train2, alfa=0.00001, maxIt=4, variables=train2.columns, mono=False)
print("\nTest: \n", test2)
print("\nPredicción: \n")
acc = 0
for i in todos_W2:
